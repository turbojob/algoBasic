...
public class Code01_FindMinKth {
	...
	public static int bfprt(int[] arr, int L, int R, int index){
		...
		int[] num = MyPartition(arr, L, R, pivot);
		if(index >= num[0] && index <= num[1])
			return arr[num[0]];
		else if(index < num[0])
			return bfprt(arr, L, num[1]-1, index);
		else
			return bfprt(arr, num[0]+1, R, index);
	...
	public static int getMedian1(int[] arr, int L, int R) {
		insertionSort(arr, L, R);
		return arr[(L + R) / 2];
	}
